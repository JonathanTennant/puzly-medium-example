...
class PlayBoardViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        puzlyBrain.setup(imageToTile: recievedImage!)
        puzlyBrain.delegate = self
        ...
        for i in 0...puzlyBrain.numberOfTiles - 1 {

            imageCollection[i].image = puzlyBrain.imageTiles[i].image
        }
    ...
    @objc func handlePan(sender: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            puzlyBrain.updateBrain(viewCollection: viewCollection, imageCollection: imageCollection, backViews: backViews, touchedImage: touchedImage)
            shadowView.alpha = 0
    ...
    func visualShuffle() {
        ...
        loop = { [weak self] count in
          guard count > 0 else { return }
            self!.puzlyBrain.imageTiles.shuffle()
            self!.impactGenerator.impactOccurred(intensity: 50)
            for i in 0...self!.puzlyBrain.numberOfTiles - 1 {
                self!.imageCollection[i].image = self!.puzlyBrain.imageTiles[i].image
            }
